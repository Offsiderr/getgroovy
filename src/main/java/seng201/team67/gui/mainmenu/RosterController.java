...
public class RosterController {
    ...
    private void populateLineup() {
        ...
        for (int i = 0; i < lineup.size(); i++) {
            ...
            int slotIndex = i;
            ArtistDetailBoxFiller.addActionButton(card, "Remove from Lineup", () -> removeFromLineup(card, slotIndex));
    ...
    private void populateAllArtists() {
        ...
        for (Artist artist : gameEnvironment.getLabelService().getAllArtists()) {
            ...
            ArtistDetailBoxFiller.addActionButton(card, "Add to Lineup", () -> addToLineup(card));
    ...
    private VBox createCard(Artist artist) {
        VBox root = ArtistDetailBoxFiller.createArtistBox(artist, dragString -> {
            ...
            boolean unequipped = gameEnvironment.getLabelService().unequipItem(artist, item);
            if (unequipped) {
                refreshView();
            }
        }, null);
